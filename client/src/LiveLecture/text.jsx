...
export default function LiveLecture() {
  ...
  const startScreenShare = async () => {
    try {
      ...
      // Replace the user's video stream with the screen share stream for all peers
  ...
  const stopScreenShare = () => {
    if (screenStream) {
      ...
      // Restore the user's video stream for all peers
      Object.values(peers).forEach((peer) => {
        const senders = peer.peerConnection.getSenders();
        const videoSender = senders.find((sender) => sender.track.kind === "video");
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900 text-white">
      <div className="flex-1 grid grid-cols-3 gap-4 p-4" ref={videoGridRef}>
